...
const fetchFacets = async (pick) => {
  ...
  const authors =
    pick.type === "tv"
      ? (data.created_by || []).map((person) => person.id)
      : crew.filter((member) => member.job === "Director").map((member) => member.id);

  const topKeywords = keywords.slice(0, KEYWORDS_PER_TITLE);
  const topCast = cast.slice(0, CAST_PER_TITLE);
  const authorPeople = pick.type === "tv" ? data.created_by || [] : crew.filter((m) => m.job === "Director");
...
export const buildTasteProfile = async (picks, { fallbackGenreIds = [] } = {}) => {
  ...
  return {
    ...
    genres: genreVector,
    keywords: finalize(keywords, VECTOR_CAP.keywords),
    people: finalize(people, VECTOR_CAP.people),

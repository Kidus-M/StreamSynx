...
export const scoreMatch = (mine, theirs, { idf, mutualCount = 0 } = {}) => {
  ...
  // once for being thin, not squared into irrelevance. Multiplying them was
  // enough to sink a pair sharing real picks below a generic genre twin.
  const confidence = Math.sqrt(confidenceOf(mine.pickCount) * confidenceOf(theirs.pickCount));
 ...
 *
 * Pass `idf` when ranking twice. Rarity has to be measured against the whole
 * pool: rebuilt over a shortlist of already-similar people it would call their
 * shared traits common, quietly changing the percentages between passes.
 */
export const rankMatches = (
  me,
  candidates,
  { excludeIds = new Set(), mutualCounts = {}, minScore = MIN_MATCH_SCORE, limit = 40, idf } = {}
) => {
  ...
  const rarityTables = idf || buildIdf([mine, ...pool.map((candidate) => candidate.tasteProfile)]);

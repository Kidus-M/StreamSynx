...
export const createUserDocuments = async (user, username) => {
  await setDoc(doc(db, "users", user.uid), {
    ...
    // Taste matching reads these off the user document; seeding them keeps
    // Discover's `hasTastePicks` query well defined for brand new accounts.
    tastePicks: { movies: [], shows: [] },
    hasTastePicks: false,
  });

...
export const buildViewerContext = async (uid) => {
  ...
  // Everything they already know: watched, loved, picked as "them", or
  // mid-way through. The server drops these even if the model suggests them.
  const seen = new Set(
    [
      ...movies.map((entry) => `movie:${entry.id}`),
      ...episodes.map((entry) => `tv:${entry.tvShowId}`),
      ...(favorites?.movies || []).map((entry) => `movie:${entry.id}`),
      ...(favorites?.episodes || []).map((entry) => `tv:${entry.tvShowId}`),
      ...picks.movies.map((pick) => `movie:${pick.id}`),
      ...picks.shows.map((pick) => `tv:${pick.id}`),
      ...continueWatching.map((entry) => `${entry.media_type}:${entry.id}`),
    ].filter((key) => !/:(undefined|null|)$/.test(key))
  );

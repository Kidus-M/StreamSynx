...
import { buildIdf, rankMatches } from "./tasteMatch";
...
export const discoverBuddies = async (me, { excludeIds = new Set(), limit = 30 } = {}) => {
  ...
  const idf = buildIdf([me.tasteProfile, ...pool.map((candidate) => candidate.tasteProfile).filter(Boolean)]);

  const firstPass = rankMatches(me, pool, { excludeIds, idf, limit: SOCIAL_SHORTLIST });
  ...
  const matches = rankMatches(me, shortlist, { excludeIds, mutualCounts, idf, limit });

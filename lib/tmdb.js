...
import { getLastEpisode } from "./localStore";
...
/**
 * Canonical watch route for any TMDB item. Series resume at the last episode
 * watched on this device, falling back to S1 E1.
 */
export const watchHref = (item) => {
  if (mediaTypeOf(item) !== "tv") return `/watch?movie_id=${item.id}`;

  const resume = getLastEpisode(item.id);
  return `/watchTv/${item.id}/${resume?.season || 1}/${resume?.episode || 1}`;
};

...
export const chooseNudge = ({ pathname, signedIn, hasTastePicks = null, now = Date.now() }) => {
  ...
  const match = NUDGES.find((nudge) => {
    ...
    if (state.goals[nudge.id]) return false;
...
/**
 * Whether any nudge could fire here at all, ignoring the per-nudge condition.
 * The caller uses this to avoid a Firestore read for someone whose prompts are
 * spent, snoozed or out of session anyway.
 */
export const hasEligibleNudge = ({ pathname, signedIn, now = Date.now() }) => {
  if (isQuietRoute(pathname) || sessionSpent()) return false;

  const state = readState();
  if (state.lastShownAt && now - state.lastShownAt < GLOBAL_GAP_MS) return false;

  const audience = signedIn ? "user" : "guest";

  return NUDGES.some((nudge) => {
    if (nudge.audience !== audience) return false;
    if (nudge.hideOn.some((route) => pathname === route || pathname.startsWith(`${route}/`))) {
      return false;
    }
    ...
    const item = itemOf(state, nudge.id);
    return !item.off && item.shows < nudge.maxShows && !(item.lastAt && now - item.lastAt < nudge.cooldownMs);
  });
};

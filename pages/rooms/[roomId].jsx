...
const RoomPage = () => {
 ...
 useEffect(() => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(roomDocRef, async (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (!userIsCurrentlyMember && data.createdBy !== currentUser.uid) {
                    ...
                    setRoomData(null); setIsMember(false); setMembersList([]); setLoadingRoom(false);
                ...
                if (data.members && Array.isArray(data.members)) {
                    try {
                        ...
                        const currentMemberIds = membersList.map(m => m.uid).sort().join(',');
                        const newMemberIds = [...data.members].sort().join(','); // Create sorted copy

                        if (currentMemberIds !== newMemberIds) {
                            const memberDocsPromises = data.members.map(uid => getDoc(doc(db, "users", uid)));
                            const memberDocs = await Promise.all(memberDocsPromises);
                            setMembersList(
                                memberDocs.filter(d => d.exists()).map(d => ({ uid: d.id, username: d.data().username || 'Anonymous' }))
                            );
                        }
                    } catch (err) {
                ...
                setRoomData(null); setIsMember(false); setMembersList([]);
            }
    ...
    useEffect(() => {
        if (roomData?.currentMediaType === 'tv' && roomData.currentMediaId) {
            ...
            if (!tvShowDetails || tvShowDetails.id !== roomData.currentMediaId) {
                ...
                fetchTMDB(`tv/${roomData.currentMediaId}`).then(details => {
                    if (details) {
                        setTvShowDetails(details);
                        // Determine initial season to load (use saved, or first available, or null)
                        const seasonToLoad = roomData.currentSeason !== null
                                            ? roomData.currentSeason
                                            : details.seasons?.find(s => s.season_number >= 1 && s.episode_count > 0)?.season_number ?? null; // Prefer Season 1+
                        setSelectedSeason(seasonToLoad); // This will trigger the episode fetch effect
                    } else { // Handle API error or no details found
                        toast.error(`Could not load details for TV show ID: ${roomData.currentMediaId}`);
                        setTvShowDetails(null); setSelectedSeason(null); setEpisodes([]);
                    }
                }).catch(err => {
            ...
            else if (tvShowDetails && tvShowDetails.id === roomData.currentMediaId && selectedSeason !== roomData.currentSeason) {
                 setSelectedSeason(roomData.currentSeason);
            }
        } else {
            // Clear TV state if media is not TV or no media selected
            setTvShowDetails(null); setSelectedSeason(null); setSelectedEpisode(null); setEpisodes([]);
        }
    // Depend on the specific properties from roomData that affect TV details fetching
    }, [roomData?.currentMediaId, roomData?.currentMediaType, roomData?.currentSeason]);

    // --- Fetch Episodes for Selected Season ---
    useEffect(() => {
        // Fetch only if it's a TV show, an ID exists, and a season number is selected
        if (roomData?.currentMediaType === 'tv' && roomData.currentMediaId && selectedSeason !== null) {
            ...
            fetchTMDB(`tv/${roomData.currentMediaId}/season/${selectedSeason}`)
                .then(seasonData => {
                    ...
                    if (Array.isArray(fetchedEpisodes)) {
                        ...
                        const isValidEpisode = fetchedEpisodes.some(ep => ep.episode_number === firestoreEpisode);
                        ...
                        console.warn(`No valid episodes array returned for S${selectedSeason}`);
                        setEpisodes([]);
                ...
                .catch(err => {
                    ...
                    // *** ADDED TOAST ERROR ***
                    toast.error(`Failed to load episodes for Season ${selectedSeason}: ${err.message}`);
                    setEpisodes([]);
    ...
    }, [roomData?.currentMediaId, roomData?.currentMediaType, selectedSeason, roomData?.currentEpisode]); // Dependencies look correct
    ...
    const performMediaSearch = useCallback(async (query) => {
        // Guard clauses
        if (!query.trim() || !currentUser || !roomData || currentUser.uid !== roomData.createdBy) {
            setSearchResults([]);
        ...
        try {
            ...
            const results = await fetchTMDB(`search/multi?query=${encodeURIComponent(query)}&page=1&include_adult=false`);

            if (results?.results) {
                // Filter results *after* getting them
                setSearchResults(results.results.filter(r =>
    ...
    }, [currentUser, roomData]); // roomData dependency is important here for createdBy check
    ...
    const handleSelectMedia = async (media) => {
        ...
        const selectToastId = toast.loading(`Selecting ${media.title || media.name}...`);
        ...
        try {
            ...
            toast.success(`Set room media to "${media.title || media.name}"`, { id: selectToastId });
            // Clear search state
            setSearchQuery(''); setSearchResults([]); setIsSearchingMedia(false);
            // Clear local TV state immediately for faster UI update
            setTvShowDetails(null); setSelectedSeason(null); setSelectedEpisode(null); setEpisodes([]);
        } catch (error) {
            console.error("Error updating media:", error);
            toast.error(`Failed to update room media: ${error.message}`, { id: selectToastId });
    ...
    const handleSeasonChange = async (seasonNumber) => {
        if (!roomId || !currentUser || !roomData || roomData.currentMediaType !== 'tv') return;
        ...
        if (currentUser.uid !== roomData.createdBy) {
            ...
            seasonSectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); // Scroll efficiently
            return;
        }
        ...
        try {
            ...
            seasonSectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        } catch (error) {
    ...
    const handleEpisodeClick = async (episodeNumber) => {
        if (!roomId || !currentUser || !roomData || selectedSeason === null || roomData.currentMediaType !== 'tv') return;
...
const performUserSearch = useCallback(async (query) => {
    ...
    const calculateVideoSrc = useCallback(() => {
        ...
        return url;
    }, [roomData?.currentMediaId, roomData?.currentMediaType, selectedSeason, selectedEpisode]);
    ...
    if (!currentUser || !roomData) {
         return ( <div className="min-h-screen bg-primary text-textprimary flex flex-col items-center justify-center px-4"> <NavBar /> <div className="text-center mt-20"> <h2 className="text-2xl text-red-500 mb-4">Access Denied</h2> <p className="text-textsecondary mb-6">Could not load room data or user information.</p> <button onClick={() => router.push('/rooms')} className="bg-accent hover:bg-accent-hover text-primary font-semibold py-2 px-6 rounded-lg transition-colors"> Back to Rooms </button> </div> <Footer /> </div> );
    }
        ...
        <div className="min-h-screen bg-primary text-textprimary flex flex-col font-poppins">
            ...
            {roomData.currentMediaPoster && (
                <div className="absolute top-0 left-0 w-full h-[50vh] md:h-[60vh] -z-10 overflow-hidden" aria-hidden="true">
                    <img src={`${IMAGE_BASE_URL_ORIGINAL}${roomData.currentMediaPoster}`} alt="" className="w-full h-full object-cover opacity-20 blur-md scale-110"/>
            ...
            <main className="flex-1 container mx-auto p-4 md:p-6 lg:p-8 mt-16 space-y-6 md:space-y-8 pt-20 md:pt-24">
                ...
                <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-accent flex items-center gap-2">
                            {roomData.name}
                            {isCreator && <FaCrown className="text-yellow-400 text-xl" title="You are the creator" />}
                        </h1>
                        <p className="text-sm text-textsecondary">Created by {roomData.createdByUsername}</p>
                        {/* Member List Display (Improved) */}
                        <div className="flex items-center gap-1 text-textsecondary mt-1 text-xs" title={membersList.map(m => m.username).join(', ')}>
                            <FaUsers /> {membersList.length} member{membersList.length !== 1 ? 's' : ''}
                            {membersList.length > 0 && membersList.length <= 5 && `: ${membersList.map(m => m.username).join(', ')}`}
                            {membersList.length > 5 && ` (including ${membersList.slice(0, 2).map(m => m.username).join(', ')}...)`}
                        </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        {isCreator && (
                            <motion.button whileTap={{ scale: 0.95 }} onClick={() => setShowUserSearch(!showUserSearch)} className="bg-secondary-light hover:bg-secondary text-textprimary font-semibold py-1.5 px-3 rounded-lg transition-colors text-sm flex items-center gap-1.5">
                                <FaUserPlus /> {showUserSearch ? 'Close Add Member' : 'Add Member'}
                            </motion.button>
                        )}
                        <motion.button whileTap={{ scale: 0.95 }} onClick={handleLeaveRoom} disabled={isCreator} className={`bg-red-600/20 hover:bg-red-600/40 text-red-400 font-semibold py-1.5 px-3 rounded-lg transition-colors text-sm flex items-center gap-1.5 ${isCreator ? 'opacity-50 cursor-not-allowed' : ''}`} title={isCreator ? "Creators cannot leave directly" : "Leave Room"}>
                            <FaSignOutAlt /> Leave
                        </motion.button>
                    </div>
                </section>
                ...
                <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
                    ...
                    <div className="w-full lg:w-2/3 space-y-6">
                        {/* Player */}
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full rounded-lg overflow-hidden shadow-lg bg-black aspect-video border border-secondary-light">
                            {videoSrc ? (
                                <iframe
                                    ...
                                    title={`Watch Party Player - ${roomData.currentMediaTitle || 'Select Media'}`}
                                ></iframe>
                            ) : (
                                <div className="w-full h-full flex flex-col items-center justify-center text-textsecondary p-4">
                                    <p className="mb-4 text-center">No movie or TV show selected.</p>
                                    {isCreator && <p className="text-sm text-center">Use the search below to select media.</p>}
                                </div>
                            )}
                        </motion.div>
                        ...
                        {roomData.currentMediaId && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="bg-secondary rounded-lg p-3 flex items-center gap-3 shadow">
                                {roomData.currentMediaPoster && <img src={`${IMAGE_BASE_URL_W500}${roomData.currentMediaPoster}`} alt="Poster" className="w-12 h-auto rounded flex-shrink-0"/>}
                                <div className="flex-grow">
                                    <p className="text-sm text-textsecondary">Currently Watching:</p>
                                    <p className="font-semibold text-textprimary text-lg">{roomData.currentMediaTitle || `(${roomData.currentMediaType})`}</p>
                                    {roomData.currentMediaType === 'tv' && selectedSeason !== null && selectedEpisode !== null && <p className="text-xs text-accent">S{selectedSeason}, E{selectedEpisode}</p>}
                                </div>
                            </motion.div>
                        )}
                        ...
                        {isCreator && (
                            <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
                                <label className="text-lg font-semibold mb-3 text-textprimary block">Select Media</label>
                                ...
                                {(searchResults.length > 0 || (isSearchingMedia && searchQuery)) && (
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-80 overflow-y-auto p-2 bg-secondary/50 rounded-md scrollbar-thin scrollbar-thumb-secondary-light scrollbar-track-secondary">
                                        {isSearchingMedia && searchResults.length === 0 && <p className="text-textsecondary italic col-span-full text-center py-4">Searching...</p>}
                                        {searchResults.map((media) => (
                                            // Extracted Search Result Card (Example Structure)
                                            <div key={media.id} className="bg-primary rounded-lg overflow-hidden shadow group relative cursor-pointer" onClick={() => handleSelectMedia(media)}>
                                                 <img src={media.poster_path ? `${IMAGE_BASE_URL_W500}${media.poster_path}` : '/placeholder.jpg'} alt={media.title || media.name} className="w-full h-auto object-cover aspect-[2/3]" loading="lazy" />
                                                 <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-end p-2 text-center">
                                                     <p className="text-xs font-semibold text-textprimary line-clamp-2 mb-1">{media.title || media.name}</p>
                                                     <p className="text-xs text-accent uppercase mb-2">{media.media_type} {media.release_date?.substring(0,4) || media.first_air_date?.substring(0,4)}</p>
                                                  ...
                                                  <div className="absolute top-1 right-1 bg-accent/80 text-primary text-[10px] font-bold px-1 rounded-sm">
                                                     <FaStar className="inline mr-0.5 mb-px" size={8}/> {media.vote_average?.toFixed(1)}
                                                  </div>
                                             </div>
                                        ))}
                                    </div>
                                ...
                                {!isSearchingMedia && searchQuery && searchResults.length === 0 && <p className="text-xs text-textsecondary text-center mt-2">No results found for "{searchQuery}".</p>}
                            </motion.section>
                        )}

                        {/* TV Show Season/Episode Selection */}
                        {roomData.currentMediaType === 'tv' && tvShowDetails && (
                            <motion.section ref={seasonSectionRef} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="bg-secondary rounded-lg shadow-lg p-4 md:p-6">
                                {/* Season Selector */}
                                <div className="mb-6">
                                    <h3 className="text-lg font-semibold mb-3 text-textprimary">Seasons</h3>
                                    <div className="flex flex-wrap gap-2">
                                        {(tvShowDetails.seasons || []).filter(s => s.season_number !== 0 && s.episode_count > 0).map((season) => (
                                            <button key={season.id} onClick={() => handleSeasonChange(season.season_number)} className={`px-3 py-1 rounded-md text-xs font-medium transition-colors duration-200 ${ selectedSeason === season.season_number ? "bg-accent text-primary shadow-md" : "bg-secondary-light text-textsecondary hover:bg-secondary-light/70 hover:text-textprimary"}`}>
                                ...
                                {/* Episode List */}
                                <div>
                                    <h3 className="text-lg font-semibold mb-3 text-textprimary">Episodes {selectedSeason !== null ? `(Season ${selectedSeason})` : ""}</h3>
                                    {loadingEpisodes ? (
                                        <div className="flex justify-center items-center h-20"><Mosaic color="#DAA520" size="small" /></div>
                                    ) : episodes.length > 0 ? (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-secondary-light scrollbar-track-secondary pr-2">
                                            {episodes.map((episode) => (
                                                ...
                                                <EpisodeCard
                                                    ...
                                                    theme={{ accent: '#DAA520', primary: '#...', secondary: '#...', textPrimary: '#...', textSecondary: '#...' }}
                                                />
                                            ))}
                                        </div>
                                    ) : selectedSeason !== null ? (
                                        <p className="text-textsecondary italic text-sm">No episodes found for this season.</p>
                                    ) : (
                                        <p className="text-textsecondary italic text-sm">Select a season to view episodes.</p>
                                    )}
                                </div>
                            </motion.section>
                        )}
                        ...
                        {isCreator && (
                            <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="bg-secondary rounded-lg shadow-lg p-4 md:p-6">
                                <h3 className="text-lg font-semibold mb-3 text-textprimary">Manage Members ({membersList.length})</h3>
                                <ul className="space-y-2 max-h-60 overflow-y-auto text-sm pr-2 scrollbar-thin scrollbar-thumb-secondary-light scrollbar-track-secondary">
                                    {membersList.map(member => (
                                        <li key={member.uid} className="flex justify-between items-center p-2 bg-primary rounded">
                                            <span className="flex items-center gap-1.5">
                                                {member.username}
                                                {member.uid === roomData.createdBy && <FaCrown className="text-yellow-400" title="Creator"/>}
                                            </span>
                                            {/* Allow removing anyone except the creator */}
                                            {member.uid !== roomData.createdBy && (
                                                <button onClick={() => handleRemoveMember(member.uid)} className="text-red-500 hover:text-red-400 text-base p-1" title={`Remove ${member.username}`}><FaUserMinus /></button>
                                            )}
                                        </li>
                                    ))}
                                </ul>
                    ...
                    <div className="w-full lg:w-1/3">
                         <div className="lg:sticky lg:top-24"> {/* Make chat sticky on large screens */}

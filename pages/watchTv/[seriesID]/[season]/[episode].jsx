...
const TVShowPlayerPage = () => {
    ...
    const seriesId = router.isReady ? seriesID || null : null;

    const { tvShow, loading: loadingShow, error } = useTVShow(seriesId, apiKey);
    const { recommendedShows } = useRecommendedShows(seriesId, apiKey);
    ...
    // Set default route if no season or episode
    useEffect(() => {
        if (router.isReady && seriesID && (!season || !episode)) {
            router.replace(`/watchTv/${seriesID}/1/1`, undefined, { scroll: false });
        }
    }, [router.isReady, seriesID, season, episode, router]);

    // Set selected season and episode from query params
    useEffect(() => {
        if (router.isReady && season && episode) {
            setSelectedSeason(parseInt(season));
            setSelectedEpisode(parseInt(episode));
        }
    }, [router.isReady, season, episode]);
            ...
            try {
                ...
                setIsFavorite(
                    ...
                    (favoritesDoc.data().episodes || []).some(
                        (ep) => ep.tvShowId === parseInt(seriesId)
                    )
                );
    ...
    const handleEpisodeClick = (episodeNumber) => {
        ...
        router.push(`/watchTv/${seriesID}/${selectedSeason}/${episodeNumber}`, undefined, { scroll: false });
    };

    // --- Season Change Handler (keep as is) ---
    const handleSeasonChange = (seasonNum) => {
        router.push(`/watchTv/${seriesID}/${seasonNum}/1`, undefined, { scroll: false });
    };

    // --- Render States ---
    if (!router.isReady || loadingShow) {
        return (
    ...
    return (
        ...
        <div className="min-h-screen mt-16 bg-primary text-textprimary flex flex-col font-poppins">
            <Head>
                ...
                <link rel="canonical" href={`streamsynx.vercel.app/watchTv/${seriesID}/${season}/${episode}`} />
                ...
                <meta property="og:url" content={`streamsynx.vercel.app/watchTv/${seriesID}/${season}/${episode}`} />

...
const MovieDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (favoritesDoc.exists())
        setIsFavorite(
          (favoritesDoc.data().movies || []).some(
            (m) => m.id === parseInt(validId)
          )
        );
      ...
      if (ratingsDoc.exists()) {
        const movieRating = (ratingsDoc.data().ratings || []).find(
          (r) => r.movieId === parseInt(validId)
        );
      ...
      try {
        ...
        if (historyDoc.exists()) {
          ...
          if (!recentMovies.some((m) => m.id === movie.id))
            await updateDoc(historyRef, { movies: arrayUnion(movieData) });
        } else {
          await setDoc(historyRef, { movies: [movieData], episodes: [] });
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-primary text-textprimary flex flex-col font-poppins">
        ...
        <meta
          ...
          content={
            movie?.overview
              ...
              : "Discover details about movies on StreamSynx."
          }
        />
        <link rel="canonical" href={`streamsynx.vercel.app/${router.asPath}`} />{" "}
        {/* Use your site URL + current path */}
      ...
      <main className="flex-1 p-4 md:p-6 mt-16 lg:p-8 space-y-6 md:space-y-8 max-w-6xl mx-auto pt-20 md:pt-24 w-full">
        {" "}
        {/* Changed mt to pt */}
              ...
              {recommendedMovies.map((recMovie) => (
                <div key={recMovie.id} className="flex-shrink-0 w-36 md:w-44">
                  <MovieCard
                    movie={{
                      ...recMovie,
                      poster_path: `${IMAGE_BASE_URL_W500}${recMovie.poster_path}`,
                    }}

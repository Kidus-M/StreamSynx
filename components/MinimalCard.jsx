...
const MovieCard = ({ movie: initialMovie }) => {
  ...
  const [mediaType, setMediaType] = useState(
    initialMovie.media_type || (initialMovie.first_air_date ? "tv" : "movie")
  );
    ...
    const fetchMovieDetails = async () => {
      ...
      try {
        const { data } = await axios.get(
          `${BASE_URL}/${mediaType}/${initialMovie.id}`,
          {
            params: {
              api_key: API_KEY,
              language: "en-US",
    ...
    const checkIfAdded = async () => {
      if (userId && movie.id) {
        ...
        if (watchlistDoc.exists() && watchlistDoc.data().items) {
          ...
          setIsAdded(
            items.some((m) => m.id === movie.id && m.media_type === mediaType)
          );
        } else {
  ...
  return (
    ...
    >
      <img
        ...
        className="w-full h-96 sm:h-[400px] object-cover rounded-xl"
      />
      <div className="absolute bottom-0 left-0 w-full bg-primary bg-opacity-60 text-white p-3 flex justify-between items-center rounded-md">
        <div className="flex-1 overflow-hidden">
          ...
          <p className="text-xs opacity-80 truncate text-ellipsis whitespace-nowrap overflow-hidden">

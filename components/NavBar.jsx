...
function NavBar() {
    ...
    const renderNavItem = (link, isMobile = false) => {
        const isActive = router.pathname === link.path || (link.path === '/rooms' && router.pathname.startsWith('/rooms')); // Highlight for room subpages too
        return (
            <li key={link.path}>
                <Link
                    href={link.path}
    ...
    return (
        <>
            ...
            <div className="w-full fixed top-0 left-0 z-40 font-poppins">
                ...
                <nav className="flex justify-between items-center bg-primary/80 backdrop-blur-lg text-textprimary w-full px-4 sm:px-6 lg:px-8 py-3 h-16 transition-all duration-300 shadow-md border-b border-secondary-light/50">
                    ...
                    <ul className="hidden lg:flex flex-grow justify-center items-center space-x-8">
                        {/* Render all links, including the new Watch Parties one */}
            ...
            <div
                ...
                className={`fixed top-0 left-0 w-72 h-full bg-secondary shadow-xl z-50 transform transition-transform duration-300 ease-in-out lg:hidden ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
